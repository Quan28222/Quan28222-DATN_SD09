...
public interface HoaDonRepository extends JpaRepository<HoaDon, UUID> {
    ...
    @Query(value = "select * from HoaDon order by NgaySua desc", nativeQuery = true)
    List<HoaDon> getAll();
    ...
    @Query("SELECT h FROM HoaDon h WHERE (h.ma LIKE %:maHoaDon%) " +
            "AND (h.hinhThucTT LIKE %:hinhThucTT%) " +
            "AND (h.trangThai LIKE %:trangThai%) " +
            "AND ((:ngayBatDauSearch is null and :ngayKetThucSearch is null and true) or (:ngayBatDauSearch is not null and h.ngayMua >= :ngayBatDauSearch)) " +
            "AND (:ngayKetThucSearch is null or h.ngayMua <= :ngayKetThucSearch) ORDER BY h.ngaySua DESC")
    List<HoaDon> danhSachHoaDonTimKiem(
            @Param("maHoaDon") String ma,
            @Param("hinhThucTT") String hinhThucThanhToan,
            @Param("trangThai") String trangThai,
            @Param("ngayBatDauSearch") Date startDate,
            @Param("ngayKetThucSearch") Date endDate
    );
}
