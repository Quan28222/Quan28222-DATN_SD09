...
//    }
    ...
    public String sua(
        ...
        if (phieuGiamGiaRequestrequest.getGiaTriGiam() < 0) {
            model.addAttribute("giaTriGiamNegative", true);
//            model.addAttribute("sanPhamApDung", selectedProductIds);
            return "/admin/PhieuGiamGiaForm/update";
        }

        if (phieuGiamGiaRequestrequest.getSoLuong() < 0) {
            model.addAttribute("soLuongNegative", true);
//            model.addAttribute("sanPhamApDung", selectedProductIds);
            return "/admin/PhieuGiamGiaForm/update";
        }

        if (phieuGiamGiaRequestrequest.getGiaTienXetDieuKien() < 0) {
            model.addAttribute("dieuKienNegative", true);
//            model.addAttribute("sanPhamApDung", selectedProductIds);
            return "/admin/PhieuGiamGiaForm/update";
        }
...
//            return "/admin/PhieuGiamGiaForm/create";
//        }
        if (Constant.getDateNow().after(request.getNgayKetThucApDung())) {
        ...
        if (request.getGiaTienXetDieuKien() < 0) {
            model.addAttribute("dieuKienNegative", true);
            // Truy vấn danh sách sản phẩm từ cơ sở dữ liệu với phân trang
            Pageable pageable = PageRequest.of(currentPage, Constant.pageNumber);
            String textSearch = httpServletRequest.getParameter("textsearch");
