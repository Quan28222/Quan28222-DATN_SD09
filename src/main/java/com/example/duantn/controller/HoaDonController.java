...
import com.example.duantn.request.HoaDonSearch;
...
//    }
    ...
    public ResponseEntity<?> getListHoaDon(
        ...
        return ResponseEntity.of(Optional.ofNullable(hoaDonRepository.getAll()));
    }

    @PostMapping("/timKiemHoaDon")
    public ResponseEntity<?> timKiemHoaDon(
            final Model model
            , @RequestBody HoaDonSearch hoaDonSearch
    ) throws IOException, ParseException {
        String maHoaDon = hoaDonSearch.getMaHoaDon();
        String ngayBatDau = hoaDonSearch.getNgayBatDau();
        String ngayKetThuc = hoaDonSearch.getNgayKetThuc();
        String hinhThucTT = hoaDonSearch.getHinhThucTT();
        String trangThai = hoaDonSearch.getTrangThai();

        Date ngayBatDauSearch = null;
        if(!ngayBatDau.isEmpty() && ngayBatDau != null) {
            ngayBatDauSearch = Date.valueOf(ngayBatDau);
        }

        Date ngayKetThucSearch = null;
        if(!ngayKetThuc.isEmpty() && ngayKetThuc != null) {
            ngayKetThucSearch = Date.valueOf(ngayKetThuc);
        }

//        List<ChiTietSanPham> chiTietSanPhams = sanPhamCTService.danhSachSanPhamTimKiem(keyWork);
        List<HoaDon> listHoaDon = hoaDonRepository.danhSachHoaDonTimKiem(maHoaDon, hinhThucTT, trangThai, ngayBatDauSearch, ngayKetThucSearch);

        System.out.println("Kết quả trả về tìm kiếm : " + listHoaDon.size());
        return ResponseEntity.of(Optional.ofNullable(listHoaDon));
    }
